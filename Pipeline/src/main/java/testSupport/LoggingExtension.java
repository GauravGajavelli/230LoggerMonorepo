...
public class LoggingExtension implements TestWatcher, BeforeAllCallback, BeforeEachCallback {
	...
	private Path tempFilepathResolve() {
		return baseDir().resolve(sourceFolderName).resolve(testSupportPackageName).resolve(tempFolderName);
	}

	private Path filepathResolve() {
		return baseDir().resolve(sourceFolderName).resolve(testSupportPackageName);
	}

	/** Returns the project root directory, honouring the pipeline-injected system property when present. */
	private static Path baseDir() {
		String injected = System.getProperty("csse230.logger.baseDir");
		return (injected != null && !injected.isEmpty()) ? Paths.get(injected) : Paths.get("");
	}
